...
> 經三方獨立驗證（交叉相關 × 2 + 事件研究），在目前約 116 天的資料下，
> **NII / Phase 訊號與股價多為「同步」而非「領先」**，「預熱」訊號進場的報酬
> 與隨機進場相比無顯著優勢。

因此本系統定位為 **「題材熱度即時監控儀表板（同步指標）」**，
而非「擇時進場工具（領先指標）」。Phase 轉換僅描述新聞熱度變化，不代表買賣時機。

完整驗證方法與結論見 [`docs/文件3_訊號驗證報告.docx`](docs/)；
驗證可由 `scripts/lead_lag_analysis.py` 與 `scripts/event_study.py` 重跑。
...
- **訊號為同步而非領先**：詳見上方「訊號定位」，請勿將 Phase 訊號當作進出場依據
